...
@Component({
  ...
  template: `
    <nav class="navbar">
      <div class="nav-inner">
        ...
        <!-- Manual Download -->
        <button class="manual-btn" (click)="downloadManual()" title="Download instruction manual as PDF">
        ...
        <div class="excel-actions">
          ...
          <!-- Export -->
          <button class="excel-btn" (click)="exportBooks()" title="Export books to Excel">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
          ...
          <!-- Import -->
          <button class="excel-btn import-btn" (click)="triggerImport()" title="Import books from Excel">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div class="nav-stats">
          <div class="stat-pill">
            ...
            <span>{{ bookService.readingBooks().length }} reading</span>
          </div>
          <div class="stat-pill lent" *ngIf="bookService.lentBooks().length > 0">
            <span class="stat-dot lent-dot"></span>
            <span>{{ bookService.lentBooks().length }} lent</span>
          </div>
        </div>
              ...
              <li
                *ngFor="let a of visibleAlerts()"
                ...
                <div class="notif-icon">
                  <span *ngIf="a.severity === 'red'">🔴</span>
                  <span *ngIf="a.severity === 'orange'">🟠</span>
                </div>
                <div class="notif-body">
                  <div class="notif-book-title">{{ a.title }}</div>
                  <div class="notif-meta">
                    Lent to <strong>{{ a.borrowedBy }}</strong>
                    &nbsp;·&nbsp;
                    <span [class.text-red]="a.severity === 'red'" [class.text-orange]="a.severity === 'orange'">
                      {{ a.months }} months ago
                    </span>
                  </div>
                  <div class="notif-since">Since {{ a.since }}</div>
                </div>
        ...
        <!-- Logout button -->
        <button class="logout-btn" (click)="logout()" title="Lock library">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
  ...
  styles: [`
    ...
    .notif-title svg { width: 14px; height: 14px; color: #f59e0b; }
    ...
    .notif-item.severity-red {
      border-left-color: #ef4444;
      background: rgba(239,68,68,0.04);
    }
    .notif-item.severity-orange {
      border-left-color: #f97316;
      background: rgba(249,115,22,0.04);
    }

    .notif-icon { font-size: 0.85rem; margin-top: 0.1rem; flex-shrink: 0; }
    ...
    .excel-btn svg { width: 14px; height: 14px; }
    .excel-btn:hover {
      color: var(--text-main);
      background: rgba(255,255,255,0.1);
      border-color: rgba(255,255,255,0.2);
    }
    .excel-btn.import-btn:hover {
...
export class NavbarComponent {
  ...
  async onFileSelected(event: Event) {
    ...
    try {
      const { books, skipped } = await this.excelService.import(file);
